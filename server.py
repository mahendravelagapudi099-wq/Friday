...
async def analyze_vision():
    try:
        ...
        frame = vision_encoder.last_frame
        if frame is None:
            return {"analysis": None, "error": "No frame captured"}
        import base64
        frame_b64 = base64.b64encode(frame).decode()
        frame_url = "data:image/jpeg;base64," + frame_b64
        try:
            completion = client.chat.completions.create(
                model="microsoft/phi-4-vision",
                messages=[{"role": "user", "content": [{"type": "text", "text": "Describe what you see concisely in 1-2 sentences."}, {"type": "image_url", "image_url": {"url": frame_url}}]}],
                temperature=0.5,
                max_tokens=200,
            )
