...
def save_brain(brain, path=None, force=False):
    ...
    for name, region in brain.regions.items():
        ...
        if name == "hippocampus" and hasattr(region, "memory_buffer"):
            ...
            # V1.3 Fix: Save memory strength too, otherwise simulation crashes on load
            if hasattr(region, "_memory_strength"):
                reg_state["memory_strength"] = region._memory_strength[-100:]
        ...
        if os.path.exists(reg_dir):
            for name, region in brain.regions.items():
                ...
                if name == "hippocampus" and "memory_buffer" in reg_state:
                    ...
                    # V1.3 Fix: Restore memory strength to match buffer size
                    if "memory_strength" in reg_state:
                        region._memory_strength = reg_state["memory_strength"]
                    else:
                        # Fallback: initialize with 1.0s if missing from old save
                        region._memory_strength = [1.0] * len(region.memory_buffer)
