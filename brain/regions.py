...
class Hippocampus(BrainRegion):
    ...
    def step(self, external_input: np.ndarray, t: float) -> np.ndarray:
        ...
        if rate < 0.01 and len(self.memory_buffer) > 0:
            ...
            # Safety Fix: Ensure strength list matches buffer size (prevents crash on loaded brains)
            if len(self._memory_strength) != n:
                self._memory_strength = [1.0] * n
...
class FridayNet(BrainRegion):
    """The Friday Neural Network - A dedicated processing cluster for the AI persona.
    Connected to the prefrontal cortex for high-level goal alignment."""

    def __init__(self, n_neurons: int, dt: float = 1.0):
        super().__init__("friday_net", n_neurons,
                        BrainConfig.NEURON_PARAMS["friday_net"], dt)
        # Position at the back, separate from main sensory cortex
        self.position = np.array([0.0, -0.7, -0.4])
        self.goal_state = np.zeros(n_neurons)

    def step(self, external_input: np.ndarray, t: float) -> np.ndarray:
        # Friday's net has internal recurrence to maintain state
        recurrence = self.goal_state * 2.5
        fired = super().step(external_input + recurrence, t)
        self.goal_state = 0.9 * self.goal_state + 0.1 * fired.astype(np.float32)
        return fired
